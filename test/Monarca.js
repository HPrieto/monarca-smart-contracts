...
contract('Monarca', (accounts) => {
  beforeEach(async () => {
    HST = await Monarca.new(accounts[0], accounts[0], accounts[0], { from: accounts[0] });
  });

  it('creation: should create an initial balance of 1000000000 for the creator', async () => {
    const balance = await HST.balanceOf.call(accounts[0]);
    assert.strictEqual(balance.toNumber(), 1000000000);
  });

  it('creation: test correct setting of vanity information', async () => {
    const name = await HST.name.call();
    assert.strictEqual(name, 'Monarca');

    // const decimals = await HST.decimals.call();
    // assert.strictEqual(decimals.toNumber(), 1);

    const symbol = await HST.symbol.call();
    assert.strictEqual(symbol, 'MONA');
  });
  ...
  it('transfers: ether transfer should be reversed.', async () => {
    ...
    assert.strictEqual(balanceBefore.toNumber(), 1000000000);
    ...
    assert.strictEqual(balanceAfter.toNumber(), 1000000000);
  });

  // TRANSFERS


  it('transfers: should transfer 1000000000 to accounts[1] with accounts[0] having 69420000000000', async () => {
    await HST.transfer(accounts[1], 1000000000, { from: accounts[0] });
    const balance = await HST.balanceOf.call(accounts[1]);
    assert.strictEqual(balance.toNumber(), 1000000000);
  });

  it('transfers: should fail when trying to transfer 1000000001 to accounts[1] with accounts[0] having 69420000000000', async () => {
    await assertRevert(HST.transfer.call(accounts[1], 1000000001, { from: accounts[0] }));
  });
  ...
  it('approvals: msg.sender approves accounts[1] of 100 & withdraws 20 once.', async () => {
    ...
    assert.strictEqual(balance0.toNumber(), 1000000000);
    ...
    assert.strictEqual(balance02.toNumber(), 1000000000 - 20);
  });
  ...
  it('approvals: msg.sender approves accounts[1] of 100 & withdraws 20 twice.', async () => {
    ...
    const balance0 = await HST.balanceOf.call(accounts[0]);
    assert.strictEqual(balance0.toNumber(), 1000000000 - 20);
    ...
    const balance02 = await HST.balanceOf.call(accounts[0]);
    assert.strictEqual(balance02.toNumber(), 1000000000 - 40);
  });
  ...
  it('approvals: msg.sender approves accounts[1] of 100 & withdraws 50 & 60 (2nd tx should fail)', async () => {
    ...
    const balance0 = await HST.balanceOf.call(accounts[0]);
    assert.strictEqual(balance0.toNumber(), 1000000000 - 50);
